...
function CreateRegistrationForm() {
  ...
  useEffect(() => {
    ...
    if (savedEventId) {
      const link = `https://www.aurelionfutureforge.com/stagyn/${formattedEventName}/book-register/${savedEventId}`;
      console.log(link);
      setFormLink(link);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const link = `https://www.aurelionfutureforge.com/stagyn/${formattedEventName}/book-register/${eventId}`;
      setFormLink(link);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-black to-gray-800 flex items-center justify-center p-6">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-3xl">
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:gap-6 md:gap-8">
          <button
            type="button"
            onClick={addField}
            className="bg-red-600 text-white px-4 py-2 rounded-xl hover:bg-red-700 transition w-full sm:w-auto"
          >
            Add Another Field
          </button>
